...
public interface UpdateScoreAfterEveryBallRepository extends JpaRepository<ScoreAtParticularOver, OversId> {
    @Query(
            value = "select * from score_at_particular_over WHERE (match_id=:m AND team_name=:t and overs between :o-0.01 and :o+0.01)",
            nativeQuery = true
    )
    List<ScoreAtParticularOver> getScoreAt(@Param("m") int m, @Param("t") String t, @Param("o") float o);
